...
CREATE OR REPLACE TABLE `mci506-futbol-europeo.liga_gold.gold_league_kpis` AS
SELECT 
    liga,
...
FROM `mci506-futbol-europeo.liga_silver.silver_matches`
WHERE estado = 'FINISHED'
GROUP BY liga
ORDER BY total_goals_scored DESC;

-- Tabla 2: Rendimiento Histórico por Equipo (Local)
CREATE OR REPLACE TABLE `mci506-futbol-europeo.liga_gold.gold_team_performance_home` AS
SELECT 
